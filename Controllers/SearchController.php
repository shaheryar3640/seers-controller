...
class SearchController extends Controller
{
...

    {
        ...
        return view('old.search')->with($data);
    }
    ...
    public function sangvish_homeindex($id)
    {
        ...
        if(count($services) > 0){
            $subsearches = DB::table('shop')
                ...
                ->where('seller_services.subservice_id', '=', $services[0]->id)
                ->groupBy('shop.id')
                ->get();
            ...
            $data = array('title' => $title, 'subsearches' => $subsearches, 'viewservices' => $viewservices, 'shopview' => $shopview, 'sub_value' => $sub_value);
            return view('old.search')->with($data);
        }
    ...
    public function sangvish_search(Request $request)
    {
        ...
        if(!empty($datas["langOpt"]))
        {
            ...
            {
                $viewname= DB::table('subservices')->where("id", "=" , $langs)->get();
        ...
        return view('old.shopsearch')->with($data);
    }
