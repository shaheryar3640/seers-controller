...
//        File::put(public_path('/scripts/'.$fileName),$data);
    ...
    public function uploadLogo(Request $request){
        ...
        $dialogue = CookieXrayDialogue::firstOrNew(
            ...
            $script = CookieXrayScript::where('dialogue_id',$dialogue->id)->select('id','key')->first();
...
//                    $constraints->aspectRatio();
    ...
    {
        cache()->forget('script_'.$key.'');
        cache()->forget('user_'.$key.'');
        cache()->forget('dialogue_'.$key.'');
        cache()->forget('banner_'.$key.'');
        cache()->forget('firstabove-language_'.$key.'');
        cache()->forget('first-language_'.$key.'');
        cache()->forget('firstlower-language_'.$key.'');
        cache()->forget('second-language_'.$key.'');
        cache()->forget('secondlower-language_'.$key.'');
        cache()->forget('third-language_'.$key.'');
        cache()->forget('thirdlower-language_'.$key.'');
        cache()->forget('fourth-language_'.$key.'');
        cache()->forget('else-language_'.$key.'');
        cache()->forget('lower-language_'.$key.'');
        cache()->forget('categories_'.$key.'');
        cache()->forget('cookies_'.$key.'');
        return ;
    ...
    public function createCookie(Request $request)
    {
        if($request->get('key') != null){
            $this->clearCache($request->get('key'));
        }
        $cookieObj = $request->get('cookie');
        $cookie = CbCookies::where([
            ['dom_id', '=', $request->get('domain_id')],
            ['name', '=', $cookieObj["name"]],
        ])->first();
